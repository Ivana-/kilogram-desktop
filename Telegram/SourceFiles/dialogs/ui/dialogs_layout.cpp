...
void PaintRow(
		...
		const bool paintBackgroundOnly) { // kg
	...
	if (paintBackgroundOnly) return; // kg - for search result item from blocked user
...
void RowPainter::Paint(
    ...
    // // kg - for left list with variable height
    // const auto paintBackgroundOnly = true
	//     && item
	//     && item->from()
	//     && history
	//     && history->session().kgModeAndUserIsBlocked(item->from().get()->id.value);
...
void RowPainter::Paint(
    ...
    // kg - for search result item from blocked user
    const auto paintBackgroundOnly = row->searchInChat()
	    && item
	    && item->from()
	    && history
	    && history->session().kgModeAndUserIsBlocked(item->from().get()->id.value);
