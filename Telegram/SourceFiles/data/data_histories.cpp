...
// kg begin
void Histories::kgRefreshAll(bool invalidateKgData) {
	for (const auto &[peerId, history] : _map) {
		if (history->peer.get()->isUser()) continue;
		if (history->peer.get()->isBlocked()) continue;
		history->kgRefreshAll(invalidateKgData);
	}
}
// kg end
