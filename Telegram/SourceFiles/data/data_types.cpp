...
// kg begin
BareId AuthorIDFromMessage(const MTPmessage &message) {
	return message.match([](const MTPDmessageEmpty &) {
		return PeerId(0).value;
	}, [](const auto &data) {
		if (data.vflags().v.value() & (1ULL << 8)) { // https://core.telegram.org/constructor/message
			return peerFromMTP(*data.vfrom_id()).value;
		} else
		 	return BareId(0);
	});
}
