...
// kgbegin
void SessionController::toggleKgMode() {
	session().toggleKgMode();
	const auto kgMode = session().kgMode();
	// const auto window = widget().get();
	const auto widget = content().get();
	widget->resizeToWidth(widget->width() + (kgMode ? 1 : -1));
