...
, _parent(parent)
, _outer(_parent)
, _kg(&_outer, QString(), st::windowFiltersMainMenu) // kg
...
void FiltersMenu::setup() {
	...
	_parent->heightValue(
		...
		// kg begin
		_kg.resizeToWidth(width);
		_kg.move(0, 0);
		// kg end
		_menu.resizeToWidth(width);
		_menu.move(0, _kg.height()); // kg _menu.move(0, 0)
		// kg begin
		// _scroll.setGeometry( { 0, _menu.height(), width, height - _menu.height() });
		const auto stickyButtonsAreaHeight = _kg.height() + _menu.height();
		_scroll.setGeometry( { 0, stickyButtonsAreaHeight, width, height - stickyButtonsAreaHeight });
		// kg end
    ...
    // kg begin
	_kg.setClickedCallback([=] {
		_session->toggleKgMode();
		setupKgIcon();
	});
	// kg end
...
// kg begin
void FiltersMenu::setupKgIcon() {
	const auto kgIcon = _session->session().kgMode()
		? &st::windowFiltersKG
		: &st::windowFiltersTG;
	_kg.setIconOverride(kgIcon, kgIcon);
}
// kg end
